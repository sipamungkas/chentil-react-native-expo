...
import { getOutbounds } from '@/src/api/services/contentApi';
...
export default function OutboundScreen() {
  ...
  const [outboundsData, setOutboundsData] = useState<
  ...
  const fetchOutbounds = useCallback(
    async (pageNum: number, isRefresh = false) => {
      ...
      try {
        const result = await getOutbounds(PAGE_SIZE, pageNum);
        if (pageNum === 1) {
          setOutboundsData(result);
        } else {
          setOutboundsData((prev) => ({
            ...result,
          ...
          !loading ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyStateText}>
                No outbound activities found
              </Text>
              <Text style={styles.emptyStateSubtext}>
                Try searching for another activity or check back later.
              </Text>
            </View>
          ) : null
        }
